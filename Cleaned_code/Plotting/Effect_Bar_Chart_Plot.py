...
def plot_effect_bar_chart(name_dataframe, path_real_prices, day_to_plot,calibration_window):
    ...
    dataframe = pd.read_csv(os.path.join(path_forecasts_folder,name_dataframe))
    ...
    for h in range(24):
        ...
        bar_chart = alt.Chart(data1).mark_bar(size=12.5).encode(
            ...
            color=alt.Color('Abs_Coef:N', scale=alt.Scale(scheme='plasma'))
        )
